...
The macOS production design requires a separate TLS-enabled Compose profile, managed Docker Desktop startup and updates, explicit VM resource sizing, production storage and recovery procedures. Production selects a Docker named volume; package administration from macOS through Docker or purpose-built commands satisfies the visibility requirement. NGINX is intended to enforce the client CIDR, but this remains blocked until a LAN test proves that Docker Desktop preserves trustworthy client source addresses. Docker Desktop production use also requires an organization-approved paid entitlement; the free tier is not eligible for this enterprise workload. The package store remains derived and rebuildable from JCDS.
